...
async function main() {
    ...
    const tenant = await tenant_model_1.Tenant.findOne().lean();
    if (!tenant)
        throw new Error("No tenant found");
    const tenantId = tenant._id.toString();
    // Create or get a test patient
    let patient = await patient_model_1.Patient.findOne({ tenantId, phone: "+21699998888" });
    if (!patient) {
        patient = new patient_model_1.Patient({
            tenantId,
    ...
    const appt = new appointment_model_1.Appointment({
        tenantId,
    ...
    const updated = await appointment_service_1.appointmentService.updateStatus(appt._id.toString(), "no_show", tenantId);
    console.log(`Appointment updated status: ${updated?.status}`);
    // Verify patient metrics incremented
    const updatedPatient = await patient_model_1.Patient.findById(patient._id).lean();
    ...
    const attempts = await followup_model_1.FollowUpAttempt.find({
        tenantId,
        recoveryId: recovery._id,
    }).lean();
...
//# sourceMappingURL=test-no-show-automation.js.map