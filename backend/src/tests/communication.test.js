...
const webhook_controller_1 = require("../modules/communications/webhook.controller");
const communication_controller_1 = require("../modules/communications/communication.controller");
...
async function runTests() {
    ...
    try {
        ...
        const testTenants = await tenant_model_1.Tenant.find({ "settings.whatsappConfig.phoneNumberId": { $in: ["12345", "67890"] } });
        const allTestTenantIds = [...testTenants.map(t => t._id), tenantAId, tenantBId];
        await communication_model_1.Conversation.deleteMany({ tenantId: { $in: allTestTenantIds } });
        await communication_model_1.Message.deleteMany({ tenantId: { $in: allTestTenantIds } });
        ...
        await communication_model_1.Message.deleteMany({ providerMessageId: { $in: testWamids } });
        await communication_model_1.Conversation.deleteMany({ contactWaId: "+21699000111" });
        // Setup Test Data
        ...
        (0, webhook_controller_1.verifyWebhook)(req1, res1);
        if (res1.statusCode !== 200 || res1.body !== "1234")
        ...
        try {
            await communication_service_1.communicationService.handleWebhook(payloadInbound);
        }
        catch (err) {
            webhookError = err;
        ...
        if (webhookError)
            throw new Error('Webhook threw an error: ' + webhookError.message);
        const conv1 = await communication_model_1.Conversation.findOne({ tenantId: tenantAId, contactWaId: "+21699000111" });
        const msg1 = await communication_model_1.Message.findOne({ providerMessageId: "wamid.inbound.1" });
        if (!conv1)
            throw new Error("Conversation not created");
        if (!msg1 || msg1.content !== "Hello doctor" || msg1.direction !== "inbound")
            throw new Error("Message not created correctly");
        if (conv1.patientId?.toString() !== patientA._id.toString())
        ...
        await (0, webhook_controller_1.handleWebhookEvent)({ body: payloadStatus }, new MockResponse());
        const msgUpdate = await communication_model_1.Message.findOne({ providerMessageId: "wamid.outbound.1" });
        if (msgUpdate?.status !== "read")
        ...
        await (0, webhook_controller_1.handleWebhookEvent)({ body: payloadInbound }, new MockResponse());
        const msgsCount = await communication_model_1.Message.countDocuments({ providerMessageId: "wamid.inbound.1" });
        if (msgsCount !== 1)
            throw new Error("TEST 5 FAILED: Duplicate message created");
        ...
        try {
            await Promise.all([
                communication_model_1.Message.create({ tenantId: tenantAId, conversationId: conv1._id, direction: "inbound", status: "received", content: "test", providerMessageId: "wamid.concurrent.1" }),
                communication_model_1.Message.create({ tenantId: tenantAId, conversationId: conv1._id, direction: "inbound", status: "received", content: "test", providerMessageId: "wamid.concurrent.1" })
            ]);
            ...
            if (err.code !== 11000)
                throw new Error("Expected MongoDB duplicate key error (11000), got " + err.code);
        }
        ...
        await (0, communication_controller_1.getConversations)(reqAuthB, resAuthB);
        if (resAuthB.body.length !== 0)
            throw new Error("Tenant B should not see Tenant A conversations");
        const resAuthB2 = new MockResponse();
        await (0, communication_controller_1.getConversationMessages)(reqAuthB, resAuthB2);
        if (resAuthB2.statusCode !== 404)
            throw new Error("Tenant B should get 404 when requesting Tenant A's conversation");
        ...
        await (0, communication_controller_1.getConversations)(reqNoAuth, resNoAuth);
        if (resNoAuth.statusCode !== 403)
            throw new Error("Missing auth should return 403");
        ...
        await (0, communication_controller_1.getConversations)(reqAuthA, resAuthA);
        const jsonStr = JSON.stringify(resAuthA.body);
        if (jsonStr.includes("tokenA"))
        ...
        await (0, webhook_controller_1.handleWebhookEvent)({ body: payloadNoTenant }, resNoTenant);
        if (resNoTenant.statusCode !== 200)
            throw new Error("Should return 200 even if no tenant");
        const msgUnknown = await communication_model_1.Message.findOne({ providerMessageId: "wamid.unknown.1" });
        if (msgUnknown)
            throw new Error("Message created for unknown tenant");
        ...
        await (0, webhook_controller_1.handleWebhookEvent)({ body: payloadInvalid }, new MockResponse());
        ...
        await sendMessage(reqSendNoAuth, resSendNoAuth);
        if (resSendNoAuth.statusCode !== 403)
            throw new Error("Missing auth should return 403");
        ...
        await sendMessage(reqSendEmpty, resSendEmpty);
        if (resSendEmpty.statusCode !== 400)
            throw new Error("Empty content should return 400");
        ...
        await sendMessage(reqSendCross, resSendCross);
        if (resSendCross.statusCode !== 404)
            throw new Error("Cross tenant send should return 404");
        ...
        let lastProviderArgs = null;
        let mockWamid = "wamid.outbound.mocked." + Date.now();
        MetaWhatsAppProvider.prototype.sendMessage = async function (params, tenant) {
            providerCalledCount++;
        ...
        const countBeforeFail = await communication_model_1.Message.countDocuments();
        await sendMessage(reqSendFail, resSendFail);
        if (resSendFail.statusCode !== 502)
            throw new Error("Should return 502 on provider failure");
        const countAfterFail = await communication_model_1.Message.countDocuments();
        if (countBeforeFail !== countAfterFail)
            throw new Error("Message should NOT be saved in DB if provider fails");
        ...
        await sendMessage(reqSendSuccess, resSendSuccess);
        if (resSendSuccess.statusCode !== 200)
            throw new Error("Successful send should return 200");
        if (providerCalledCount !== 1)
            throw new Error("Provider should be called once");
        if (lastProviderArgs.params.content !== "Modified AI suggestion text")
            throw new Error("Provider received wrong content");
        const savedOutbound = await communication_model_1.Message.findOne({ providerMessageId: mockWamid });
