...
async function runIntegrationTests() {
    ...
    try {
        ...
        const recoveryAppt = await appointment_model_1.Appointment.create({
            tenantId: tenantAId,
            patientId: patientA._id,
            doctorId: "doc_main",
        ...
        const unrelatedAppt = await appointment_model_1.Appointment.create({
            tenantId: tenantAId,
            patientId: patientA._id,
            doctorId: "doc_main",
        ...
        const apptTenantB = await appointment_model_1.Appointment.create({
            tenantId: tenantBId,
        ...
        const task = await followup_service_1.followupService.createTask({
            patientId: patientA._id,
        ...
        // ================================================================
        console.log("\n▶ TEST 2 — Attempt logging updates correct Recovery/Task...");
        await recovery_service_1.recoveryService.markContacted(recovery._id.toString(), tenantAId);
        const attempt1 = await followup_service_1.followupService.logAttempt({
            recoveryId: recovery._id.toString(),
            taskId: task._id.toString(),
            channel: "phone",
            outcome: "no_answer",
        }, tenantAId);
        const updatedTask = await followup_model_1.FollowUpTask.findById(task._id);
        const updatedRecovery = await recovery_model_1.Recovery.findById(recovery._id);
        ...
        // ================================================================
        console.log("\n▶ TEST 3 — no_answer increments attempts...");
        const attempt2 = await followup_service_1.followupService.logAttempt({
            recoveryId: recovery._id.toString(),
            taskId: task._id.toString(),
            channel: "phone",
            outcome: "no_answer",
        }, tenantAId);
        const taskAfter2 = await followup_model_1.FollowUpTask.findById(task._id);
        ...
        // ================================================================
        console.log(`\n▶ TEST 4 — MAX_ATTEMPTS_DEFAULT (${followup_service_1.FOLLOWUP_CONFIG.MAX_ATTEMPTS_DEFAULT}) terminal lifecycle...`);
        ...
        const terminalRecovery = await recovery_model_1.Recovery.findById(recovery._id);
        const terminalTask = await followup_model_1.FollowUpTask.findById(task._id);
        ...
        const taskB = await followup_service_1.followupService.createTask({
            patientId: patientA._id,
            recoveryId: recoveryB._id,
            type: "cancellation_followup",
            priority: "medium",
        }, tenantAId);
        await recovery_service_1.recoveryService.markContacted(recoveryB._id.toString(), tenantAId);
        await followup_service_1.followupService.logAttempt({
        ...
        const recoveryBAfter = await recovery_model_1.Recovery.findById(recoveryB._id);
        const taskBAfter = await followup_model_1.FollowUpTask.findById(taskB._id);
        ...
        await recovery_service_1.recoveryService.markContacted(recoveryC._id.toString(), tenantAId);
        await followup_service_1.followupService.logAttempt({
        ...
        const recoveryCAfter = await recovery_model_1.Recovery.findById(recoveryC._id);
        const taskCAfter = await followup_model_1.FollowUpTask.findById(taskC._id);
        ...
        const recoveryD = await recovery_service_1.recoveryService.createOpportunity({
            patientId: patientA._id,
            type: "inactive_patient",
            priority: "low",
            estimatedValue: 80,
        }, tenantAId);
        const taskD = await followup_service_1.followupService.createTask({
            patientId: patientA._id,
            recoveryId: recoveryD._id,
            type: "inactive_reengagement",
            priority: "low",
        }, tenantAId);
        await recovery_service_1.recoveryService.markContacted(recoveryD._id.toString(), tenantAId);
        ...
        const recoveryDAfter = await recovery_model_1.Recovery.findById(recoveryD._id);
        const taskDAfter = await followup_model_1.FollowUpTask.findById(taskD._id);
        const attemptsD = await followup_model_1.FollowUpAttempt.find({ recoveryId: recoveryD._id });
        ...
        const bookedResult = await recovery_service_1.recoveryService.markBooked(recoveryB._id.toString(), recoveryAppt._id.toString(), 200, tenantAId);
        ...
        const taskBCompleted = await followup_model_1.FollowUpTask.findById(taskB._id);
        ...
        const bookedIdempotent = await recovery_service_1.recoveryService.markBooked(recoveryB._id.toString(), recoveryAppt._id.toString(), 200, tenantAId);
        ...
        const allTasksForB = await followup_model_1.FollowUpTask.find({ tenantId: tenantAId, recoveryId: recoveryB._id });
        ...
        const anotherAppt = await appointment_model_1.Appointment.create({
            tenantId: tenantAId,
            patientId: patientA._id,
            doctorId: "doc_main",
            treatment: "Facettes",
        ...
        await recovery_service_1.recoveryService.markContacted(recoveryD._id.toString(), tenantAId).catch(() => { }); // might already be contacted
        // Get current state
        const recoveryDCurrent = await recovery_model_1.Recovery.findById(recoveryD._id);
        // Transition to responded if needed
        if (recoveryDCurrent?.status === "contacted") {
            await recovery_service_1.recoveryService.markResponded(recoveryD._id.toString(), tenantAId);
        }
        await recovery_service_1.recoveryService.markBooked(recoveryD._id.toString(), anotherAppt._id.toString(), 80, tenantAId);
        // Mark recoveryAppt (linked to recoveryB) as completed
        await appointment_service_1.appointmentService.updateStatus(recoveryAppt._id.toString(), "completed", tenantAId);
        const recoveryBFinal = await recovery_model_1.Recovery.findById(recoveryB._id);
        const recoveryDFinal = await recovery_model_1.Recovery.findById(recoveryD._id);
        ...
        await appointment_service_1.appointmentService.updateStatus(unrelatedAppt._id.toString(), "completed", tenantAId);
        const recoveryCCheck = await recovery_model_1.Recovery.findById(recoveryC._id);
        const recoveryDCheck = await recovery_model_1.Recovery.findById(recoveryD._id);
        const recoveryBCheck = await recovery_model_1.Recovery.findById(recoveryB._id);
        ...
        const recoveryE = await recovery_service_1.recoveryService.createOpportunity({
            patientId: patientA._id,
            type: "follow_up_required",
            priority: "low",
        }, tenantAId);
        await recovery_service_1.recoveryService.markContacted(recoveryE._id.toString(), tenantAId);
        await recovery_service_1.recoveryService.markResponded(recoveryE._id.toString(), tenantAId);
        let test12Passed = false;
        try {
            // apptTenantB belongs to tenantB — should be rejected
            await recovery_service_1.recoveryService.markBooked(recoveryE._id.toString(), apptTenantB._id.toString(), 100, tenantAId);
        }
        catch (err) {
        ...
        const crossTenantUpdate = await appointment_service_1.appointmentService.updateStatus(apptTenantB._id.toString(), "completed", tenantAId // wrong tenant
        );
        ...
        try {
            await followup_service_1.followupService.completeTask(taskB._id.toString(), tenantAId, "Re-completing");
        }
        catch (err) {
        ...
        const noOpResult = await followup_service_1.followupService.completeTaskForRecovery(recoveryB._id.toString(), tenantAId);
        if (noOpResult !== null) {
        ...
        const firstRecovery = await recovery_model_1.Recovery.findOne({ tenantId: tenantAId, patientId: patientOverdue._id });
        ...
        await recovery_service_1.recoveryService.dismissOpportunity(firstRecovery._id.toString(), tenantAId);
        // Second detection run — should be blocked by 30-day cooldown
        await recovery_service_1.recoveryService.detectRecoveryOpportunities(tenantAId);
        const secondRecovery = await recovery_model_1.Recovery.findOne({
            tenantId: tenantAId,
            patientId: patientOverdue._id,
        ...
        const cooldownDays = followup_service_1.FOLLOWUP_CONFIG.RECOVERY_REDETECTION_COOLDOWN_DAYS;
        const pastDate = new Date();
        pastDate.setDate(pastDate.getDate() - (cooldownDays + 1)); // 31 days ago
        await recovery_model_1.Recovery.collection.updateOne({ _id: firstRecovery._id }, { $set: { updatedAt: pastDate } });
        // Re-run detection — should now be allowed
        await recovery_service_1.recoveryService.detectRecoveryOpportunities(tenantAId);
        const postCooldownRecovery = await recovery_model_1.Recovery.findOne({
            tenantId: tenantAId,
            patientId: patientOverdue._id,
            status: { $in: ["identified", "queued", "contacted", "responded", "booked"] },
        });
        ...
        const noShowAppt = await appointment_model_1.Appointment.create({
            tenantId: tenantAId,
            patientId: patientOverdue._id,
            doctorId: "doc_main",
            treatment: "Détartrage",
        ...
        if (postCooldownRecovery) {
            await recovery_service_1.recoveryService.dismissOpportunity(postCooldownRecovery._id.toString(), tenantAId);
            await recovery_model_1.Recovery.updateOne({ _id: postCooldownRecovery._id }, { $set: { updatedAt: new Date() } });
        }
        // Trigger no_show via appointment service
        await appointment_service_1.appointmentService.updateStatus(noShowAppt._id.toString(), "no_show", tenantAId);
        const noShowRecovery = await recovery_model_1.Recovery.findOne({
            tenantId: tenantAId,
            patientId: patientOverdue._id,
            type: "no_show",
        });
        ...
        const cancelAppt = await appointment_model_1.Appointment.create({
            tenantId: tenantAId,
            patientId: patientOverdue._id,
            doctorId: "doc_main",
            treatment: "Consultation",
        ...
        await appointment_service_1.appointmentService.updateStatus(cancelAppt._id.toString(), "cancelled", tenantAId);
        const cancelRecovery = await recovery_model_1.Recovery.findOne({
            tenantId: tenantAId,
            patientId: patientOverdue._id,
            type: "cancellation",
        ...
        const regRec = await recovery_service_1.recoveryService.createOpportunity({ patientId: patientA._id, type: "follow_up_required", priority: "low" }, tenantAId);
        let reg19Passed = false;
        try {
            await recovery_service_1.recoveryService.markVisited(regRec._id.toString(), 0, tenantAId);
        }
        catch (err) {
        ...
        if (!reg19Passed)
            throw new Error("TEST 19 FAILED: Phase 2.1 state machine regression detected!");
        ...
        const reg20Recovery = await recovery_service_1.recoveryService.createOpportunity({ patientId: patientB._id, type: "inactive_patient", priority: "medium" }, tenantBId);
        let reg20Passed = false;
        try {
            await followup_service_1.followupService.createTask({ patientId: patientA._id, recoveryId: reg20Recovery._id, type: "inactive_reengagement" }, tenantAId);
        }
        catch (err) {
        ...
        if (!reg20Passed)
            throw new Error("TEST 20 FAILED: Phase 3.1 cross-tenant task rejection regression detected!");
        ...
        const reg21Appt = await appointment_model_1.Appointment.create({
            tenantId: tenantAId,
            patientId: patientA._id,
            doctorId: "doc_main",
            treatment: "Soin",
        ...
        await appointment_service_1.appointmentService.updateStatus(reg21Appt._id.toString(), "no_show", tenantAId);
        await appointment_service_1.appointmentService.updateStatus(reg21Appt._id.toString(), "no_show", tenantAId); // idempotent
        const reg21Recoveries = await recovery_model_1.Recovery.find({ tenantId: tenantAId, sourceAppointmentId: reg21Appt._id });
        ...
        const reg22Patient = await patient_model_1.Patient.create({
            tenantId: tenantAId,
            firstName: "Reg",
            lastName: "Check",
            phone: "+21699000001",
        ...
        await recovery_service_1.recoveryService.detectRecoveryOpportunities(tenantAId);
        const reg22Recovery = await recovery_model_1.Recovery.findOne({ tenantId: tenantAId, patientId: reg22Patient._id });
