...
async function runP0ConcurrencyTests() {
    ...
    {
        await appointment_model_1.Appointment.deleteMany({ tenantId: tenantA });
        const appt1 = await appointment_service_1.appointmentService.createAppointment({
    ...
    console.log("\n▶ TEST 5 — Rendez-vous adjacent autorisé (09:00->09:30 vs 09:30->10:00)...");
    {
        ...
        const appt2 = await appointment_service_1.appointmentService.createAppointment({
            patientId: patientB._id,
            doctorId: docA._id.toString(),
            date: TEST_DATE,
            startTime: "09:30",
        ...
        const count = await appointment_model_1.Appointment.countDocuments({ tenantId: tenantA, date: TEST_DATE });
    ...
    {
        await appointment_model_1.Appointment.deleteMany({ tenantId: tenantA });
        const promises = Array.from({ length: 20 }, (_, i) => appointment_service_1.appointmentService
            .createAppointment({
            patientId: patientA._id,
            doctorId: docA._id.toString(),
            date: TEST_DATE,
        ...
        const countInDb = await appointment_model_1.Appointment.countDocuments({ tenantId: tenantA, date: TEST_DATE });
    ...
    console.log("\n▶ TEST 7 — Concurrence réelle avec 4 intervalles différents chevauchants simultanés...");
    {
        ...
        const intervals = [
            ...
            { startTime: "09:45", endTime: "10:15", durationMin: 30, name: "C (09:45-10:15)" },
            { startTime: "09:15", endTime: "09:45", durationMin: 30, name: "D (09:15-09:45)" },
        ];
        const promises = intervals.map((intv) => appointment_service_1.appointmentService
            .createAppointment({
            patientId: patientA._id,
            doctorId: docA._id.toString(),
            date: TEST_DATE,
            ...
            .then((doc) => ({ status: "fulfilled", name: intv.name, doc }))
            .catch((err) => ({ status: "rejected", name: intv.name, error: err.message })));
        const results = await Promise.all(promises);
        ...
        // Verify in database
        const apptsInDb = await appointment_model_1.Appointment.find({ tenantId: tenantA, date: TEST_DATE }).lean();
    ...
    {
        await appointment_model_1.Appointment.deleteMany({ tenantId: { $in: [tenantA, tenantB] } });
        const [resA, resB] = await Promise.all([
            appointment_service_1.appointmentService.createAppointment({
                patientId: patientA._id,
                doctorId: docA._id.toString(),
                date: TEST_DATE,
                startTime: "09:00",
        ...
        const countA = await appointment_model_1.Appointment.countDocuments({ tenantId: tenantA, date: TEST_DATE });
        const countB = await appointment_model_1.Appointment.countDocuments({ tenantId: tenantB, date: TEST_DATE });
    ...
    {
        await appointment_model_1.Appointment.deleteMany({ tenantId: tenantA });
        const appt = await appointment_service_1.appointmentService.createAppointment({
            patientId: patientA._id,
            doctorId: docA._id.toString(),
            date: TEST_DATE,
            startTime: "09:00",
        ...
        await appointment_service_1.appointmentService.updateStatus(appt._id.toString(), "cancelled", tenantA);
        // Now booking an overlapping slot (09:30->10:30) MUST succeed
        const newAppt = await appointment_service_1.appointmentService.createAppointment({
            patientId: patientB._id,
            doctorId: docA._id.toString(),
            date: TEST_DATE,
            startTime: "09:30",
    ...
    console.log("\n▶ TEST 11 — Protection contre les chevauchements lors de la modification...");
    {
        ...
        let errorThrown = false;
        try {
            await appointment_service_1.appointmentService.updateAppointment(appt2._id.toString(), {
                startTime: "09:30",
                endTime: "10:30",
            }, tenantA);
        }
        ...
        const updated = await appointment_service_1.appointmentService.updateAppointment(appt2._id.toString(), {
            startTime: "11:00",
            endTime: "12:00",
        }, tenantA);
    ...
    // Cleanup
    await appointment_model_1.Appointment.deleteMany({ tenantId: { $in: [tenantA, tenantB] } });
    await patient_model_1.Patient.deleteMany({ tenantId: { $in: [tenantA, tenantB] } });
    await user_model_1.User.deleteMany({ tenantId: { $in: [tenantA, tenantB] } });
...
//# sourceMappingURL=concurrency.p0.test.js.map