...
function makeScriptedService(scripts) {
    let i = 0;
    return {
...
async function runPhase610Tests() {
    ...
    try {
        ...
        const tenantCId = new mongoose_1.default.Types.ObjectId();
        ...
        await user_model_1.User.deleteMany({ tenantId: { $in: [tA, tB, tC] } });
        await patient_model_1.Patient.deleteMany({ tenantId: { $in: [tA, tB, tC] } });
        await communication_model_1.Conversation.deleteMany({ tenantId: { $in: [tA, tB, tC] } });
        await communication_model_1.Message.deleteMany({ tenantId: { $in: [tA, tB, tC] } });
        await appointment_model_1.Appointment.deleteMany({ tenantId: { $in: [tA, tB, tC] } });
        await recovery_model_1.Recovery.deleteMany({ tenantId: { $in: [tA, tB, tC] } });
        await followup_model_1.FollowUpTask.deleteMany({ tenantId: { $in: [tA, tB, tC] } });
        await tenant_model_1.Tenant.deleteMany({ _id: { $in: [tenantAId, tenantBId, tenantCId] } });
        // Clean deterministic reply ids from previous runs
        await communication_model_1.Message.deleteMany({ providerMessageId: { $regex: /^ai-reply-/ } });
        await communication_model_1.Message.deleteMany({ providerMessageId: { $regex: /^wamid-p610-/ } });
        // ── Seed tenants ─────────────────────────────────────────────────────────
        ...
        const docA = await user_model_1.User.create({
            tenantId: tA,
        ...
        const docB = await user_model_1.User.create({
            tenantId: tB,
        ...
        const docC = await user_model_1.User.create({
            tenantId: tC,
        ...
        const patientOther = await patient_model_1.Patient.create({
            tenantId: tB,
            firstName: "Alice",
            lastName: "Other",
            phone: "+33610000003",
        ...
        const patientC = await patient_model_1.Patient.create({
            tenantId: tC,
            firstName: "Sami",
            lastName: "Ben",
            phone: "+33610000004",
        ...
        const convHist = await communication_model_1.Conversation.create({
            tenantId: tA,
        ...
        const convNoDoc = await communication_model_1.Conversation.create({
            tenantId: tC, patientId: patientC._id, channel: "whatsapp", contactWaId: "+33610000111",
            pendingBookingContext: ctx10(NODOC_DATE),
        });
        const convUnstructured = await communication_model_1.Conversation.create({
            tenantId: tA, patientId: patientA._id, channel: "whatsapp", contactWaId: "+33610000112"
        });
        const convHumanFlag = await communication_model_1.Conversation.create({
        ...
        console.log("▶ TESTS 1-5 — Multi-turn conversation & history");
        {
            ...
            const appt = await appointment_model_1.Appointment.create({
                tenantId: tA,
                patientId: patientA._id,
            ...
            const svc5 = new ai_service_1.AIService(mockP5);
            ...
            await appointment_model_1.Appointment.deleteOne({ _id: appt._id });
        }
        // ========================================================================
        ...
        console.log("▶ TEST 6 — general_question → reply sent & persisted");
        {
            ...
            const svc = new ai_auto_booking_service_1.AIAutoBookingService(scripted, RECORDING);
            ...
            const outbound = await communication_model_1.Message.findOne({ providerMessageId: "ai-reply-wamid-p610-t6-1" });
            ...
            const apptCount = await appointment_model_1.Appointment.countDocuments({ tenantId: tA, patientId: patientA._id });
        ...
        console.log("▶ TEST 7 — appointment request, no date → clarification");
        {
            ...
            const convAfter = await communication_model_1.Conversation.findById(convClarifyDate._id).lean();
        ...
        console.log("▶ TEST 8 — availability request → real slots persisted");
        {
            ...
            const svc = new ai_auto_booking_service_1.AIAutoBookingService(new ai_service_1.AIService(realProvider), RECORDING);
            await svc.processInboundMessage(tA, convAvail._id.toString(), "wamid-p610-t8-1");
            const convAfter = await communication_model_1.Conversation.findById(convAvail._id).lean();
        ...
        console.log("▶ TEST 9 — confirmation → booking engine");
        {
            ...
            const appt = await appointment_model_1.Appointment.findOne({ tenantId: tA, patientId: patientA._id, date: CONFIRM_DATE, startTime: "10:00" });
            ...
            const confMsg = await communication_model_1.Message.findOne({ providerMessageId: confirmId });
            expect(!!confMsg && confMsg.status === "sent" && confMsg.direction === "outbound", "confirmation persisted");
            const convAfter = await communication_model_1.Conversation.findById(convConfirm._id).lean();
        ...
        {
            RECORDING.calls = [];
            await appointment_model_1.Appointment.deleteMany({ tenantId: tA, patientId: patientA._id });
            const scripted = makeScriptedService([
                reply({
            ...
            const convAfter = await communication_model_1.Conversation.findById(convEscalate._id).lean();
        ...
        console.log("▶ TEST 11 — unsupported request → escalation, no action");
        {
            ...
            const apptCount = await appointment_model_1.Appointment.countDocuments({ tenantId: tA, patientId: patientA._id, date: NO_DUR_DATE });
            ...
            const convAfter = await communication_model_1.Conversation.findById(convCancel._id).lean();
        ...
        console.log("▶ TEST 13 — missing duration → clarification");
        {
            ...
            expect(RECORDING.calls.length === 1, "clarification sent");
            const convAfter = await communication_model_1.Conversation.findById(convClarifyDur._id).lean();
        ...
        console.log("▶ TEST 15 — no availability → nothing invented");
        {
            ...
            await communication_model_1.Conversation.updateOne({ _id: convNoDoc._id }, { $unset: { pendingBookingContext: 1 } });
            const realProvider = new MockAIProvider();
            realProvider.response = JSON.stringify({
                reply: "Je vérifie les disponibilités...",
                intent: "appointment_availability",
            ...
            const convAfter = await communication_model_1.Conversation.findById(convNoDoc._id).lean();
        ...
        {
            RECORDING.calls = [];
            const contextBefore = await communication_model_1.Conversation.findById(convAvail._id).lean();
            ...
            const contextAfter = await communication_model_1.Conversation.findById(convAvail._id).lean();
        ...
        console.log("▶ TEST 17 — 6.8/6.9 engine idempotent reuse");
        {
            ...
            await appointment_model_1.Appointment.findOneAndUpdate({ tenantId: tA, patientId: patientA._id, date: CONFIRM_DATE, startTime: "10:00" }, {
                $set: {
                    doctorId: docA._id.toString(),
                    endTime: "10:30",
                    durationMin: 30,
            ...
            await communication_model_1.Conversation.updateOne({ _id: convConfirm._id }, { $set: { pendingBookingContext: ctx10(CONFIRM_DATE) } });
            const seededAppt = await appointment_model_1.Appointment.findOne({ tenantId: tA, patientId: patientA._id, date: CONFIRM_DATE, startTime: "10:00" }).lean();
            ...
            await communication_model_1.Message.findOneAndUpdate({ providerMessageId: `auto-booking-confirm-${seededAppt._id.toString()}` }, {
                $set: {
                    tenantId: tA,
            ...
            const appts = await appointment_model_1.Appointment.countDocuments({ tenantId: tA, patientId: patientA._id, date: CONFIRM_DATE, startTime: "10:00" });
        ...
        console.log("▶ TEST 18 — no booking without confirmation");
        {
            ...
            const appts = await appointment_model_1.Appointment.countDocuments({ tenantId: tA, patientId: patientA._id, date: MISM_DATE });
        ...
        {
            RECORDING.calls = [];
            await communication_model_1.Conversation.updateOne({ _id: convEscalate._id }, { $unset: { needsHuman: 1 } });
            const scripted = makeScriptedService([
                reply({
        ...
        console.log("▶ TEST 20 — no price hallucination");
        {
            ...
            const svc20 = new ai_service_1.AIService(mockP20);
        ...
        console.log("▶ TEST 21 — no availability hallucination");
        {
            ...
            await communication_model_1.Conversation.updateOne({ _id: convNoDoc._id }, { $set: { pendingBookingContext: ctx10(NODOC_DATE) } });
            const svc = new ai_auto_booking_service_1.AIAutoBookingService(scripted, RECORDING);
            ...
            const appts = await appointment_model_1.Appointment.countDocuments({ tenantId: tC, patientId: patientC._id });
        ...
        console.log("▶ TEST 22 — no treatment invention");
        {
            ...
            const appts = await appointment_model_1.Appointment.countDocuments({ tenantId: tA, patientId: patientA._id, date: NO_DUR_DATE });
        ...
        console.log("▶ TEST 23 — no false success on double booking");
        {
            ...
            const apptsB = await appointment_model_1.Appointment.countDocuments({ tenantId: tA, patientId: patientB._id, date: DUP_DATE });
        ...
        {
            const mockProvider = new MockAIProvider();
            const svc = new ai_service_1.AIService(mockProvider);
            try {
                ...
                expect(err instanceof ai_errors_1.ConversationNotFoundError, "cross-tenant read → 404-style error");
            }
            ...
            const svcWrite = new ai_auto_booking_service_1.AIAutoBookingService(scripted, RECORDING);
            ...
            const outs = await communication_model_1.Message.countDocuments({ conversationId: convGeneral._id, direction: "outbound", providerMessageId: "ai-reply-wamid-p610-t24-1" });
        ...
        console.log("▶ TEST 25 — patient isolation");
        {
            ...
            const appts = await appointment_model_1.Appointment.countDocuments({ patientId: patientOther._id });
        ...
        console.log("▶ TEST 26 — credentials not exposed");
        {
            for (const tt of [tA, tB, tC]) {
                ...
                const svcX = new ai_service_1.AIService(mockProv);
        ...
        console.log("▶ TEST 27 — prompt injection guarded");
        {
            ...
            const mockProv = new MockAIProvider();
            const svc27 = new ai_service_1.AIService(mockProv);
            ...
            await communication_model_1.Message.deleteOne({ providerMessageId: "wamid-p610-t27-inject" });
        }
        ...
        {
            const recoveryForeign = await recovery_model_1.Recovery.create({
                tenantId: tB,
            ...
            const recAfter = await recovery_model_1.Recovery.findById(recoveryForeign._id).lean();
            ...
            let threw = false;
            try {
                await (0, ai_action_executor_1.executeAIAction)(tA, convGeneral._id.toString(), {
                    type: "mark_recovery_contacted",
                    targetId: recoveryForeign._id.toString(),
                });
            }
        ...
        {
            const convWebhook = await communication_model_1.Conversation.create({
                tenantId: tA,
            ...
            const singletonRecording = { calls: [], failRemaining: 0, sendMessage: async (p) => { singletonRecording.calls.push(p); return { providerMessageId: "wa-dup" }; } };
            ai_auto_booking_service_1.aiAutoBookingService.aiService = fakeAI;
            ai_auto_booking_service_1.aiAutoBookingService.messagingProvider = singletonRecording;
            await communication_service_1.communicationService.handleWebhook(JSON.parse(JSON.stringify(payload)));
            await communication_service_1.communicationService.handleWebhook(JSON.parse(JSON.stringify(payload)));
            const inboundCount = await communication_model_1.Message.countDocuments({ providerMessageId: wamid });
            expect(inboundCount === 1, "duplicate webhook → exactly 1 inbound message");
            const replyCount = await communication_model_1.Message.countDocuments({ providerMessageId: `ai-reply-${wamid}` });
        ...
        console.log("▶ TEST 30 — retry after WhatsApp failure");
        {
            ...
            let msg = await communication_model_1.Message.findOne({ providerMessageId: "ai-reply-wamid-p610-t30-1" });
            ...
            const msgsAfter = await communication_model_1.Message.countDocuments({ providerMessageId: "ai-reply-wamid-p610-t30-1" });
            expect(msgsAfter === 1, "still exactly 1 outbound reply (upserted)");
            msg = await communication_model_1.Message.findOne({ providerMessageId: "ai-reply-wamid-p610-t30-1" });
        ...
        console.log("▶ TEST 31 — no duplicate outbound reply");
        {
            ...
            const msgs = await communication_model_1.Message.countDocuments({ providerMessageId: "ai-reply-wamid-p610-t31-1" });
        ...
        {
            const convRaceA = await communication_model_1.Conversation.create({
                tenantId: tA, patientId: patientA._id, channel: "whatsapp", contactWaId: "+33610000221",
                pendingBookingContext: ctx10(RACE_DATE),
            });
            const convRaceB = await communication_model_1.Conversation.create({
                tenantId: tA, patientId: patientB._id, channel: "whatsapp", contactWaId: "+33610000222",
                pendingBookingContext: ctx10(RACE_DATE),
            });
            const ra = (0, ai_action_executor_1.executeAIAction)(tA, convRaceA._id.toString(), {
                type: "book_appointment",
                targetId: patientA._id.toString(),
                booking: { date: RACE_DATE, startTime: "10:00", durationMin: 30, treatment: "Consultation" },
            });
            const rb = (0, ai_action_executor_1.executeAIAction)(tA, convRaceB._id.toString(), {
                type: "book_appointment",
                targetId: patientB._id.toString(),
                booking: { date: RACE_DATE, startTime: "10:00", durationMin: 30, treatment: "Consultation" },
            });
            ...
            const doubleBooked = [resA, resB].filter((r) => r.status === "rejected" && r.reason.message.includes("Double_Booking_Error")).length;
            ...
            const appts = await appointment_model_1.Appointment.countDocuments({ tenantId: tA, date: RACE_DATE, startTime: "10:00" });
        ...
        console.log("▶ TEST 33 — unstructured AI output never auto-sent");
        {
            ...
            const msgs = await communication_model_1.Message.countDocuments({ providerMessageId: "ai-reply-wamid-p610-t33-1" });
        ...
        console.log("▶ TEST 34 — needsHuman blocks further AI processing");
        {
            ...
            const appts = await appointment_model_1.Appointment.countDocuments({ tenantId: tA, patientId: patientA._id, date: DUP_DATE, startTime: "14:00" });
