...
async function runDetectionTests() {
  ...
  try {
    ...
    const patientInactive = await Patient.create({
      tenantId: tenantAId,
      firstName: "Claire",
      lastName: "Dupont",
      phone: "+21699111333",
    ...
    const patientWithUpcoming = await Patient.create({
      tenantId: tenantAId,
      firstName: "Florence",
      lastName: "Foresti",
      phone: "+21699777888",
    ...
    const patientNoVisits = await Patient.create({
      tenantId: tenantAId,
      firstName: "Gabriel",
      lastName: "NewLead",
      phone: "+21699000999",
    ...
    const recInactive = await Recovery.findOne({ tenantId: tenantAId, patientId: patientInactive._id });
    ...
    const taskInactive = await FollowUpTask.findOne({ tenantId: tenantAId, recoveryId: recInactive._id });
    ...
    const recOverdue = await Recovery.findOne({ tenantId: tenantAId, patientId: patientOverdue._id });
    if (!recOverdue || recOverdue.type !== "overdue_checkup") {
      throw new Error("TEST 2 FAILED: Overdue checkup Recovery not created properly");
    }
    const taskOverdue = await FollowUpTask.findOne({ tenantId: tenantAId, recoveryId: recOverdue._id });
    ...
    const recUpcoming = await Recovery.findOne({ tenantId: tenantAId, patientId: patientWithUpcoming._id });
    if (recUpcoming !== null) {
    ...
    const redetect = await recoveryService.detectRecoveryOpportunities(tenantAId);
    ...
    const allRecs = await Recovery.find({ tenantId: tenantAId });
    const allTasks = await FollowUpTask.find({ tenantId: tenantAId });
    ...
    await Promise.all([
      recoveryService.detectRecoveryOpportunities(tenantAId),
      recoveryService.detectRecoveryOpportunities(tenantAId),
    ]);
    const allRecsAfterConcurrent = await Recovery.find({ tenantId: tenantAId });
    const allTasksAfterConcurrent = await FollowUpTask.find({ tenantId: tenantAId });
    ...
    const resultB = await recoveryService.detectRecoveryOpportunities(tenantBId);
    ...
    const recNoVisits = await Recovery.findOne({ tenantId: tenantAId, patientId: patientNoVisits._id });
    if (recNoVisits !== null) {
    ...
    const patientBoundary = await Patient.create({
      tenantId: tenantAId,
      firstName: "Hugo",
      lastName: "Boundary",
      phone: "+21699888999",
    ...
    await recoveryService.detectRecoveryOpportunities(tenantAId);
    const recBoundary = await Recovery.findOne({ tenantId: tenantAId, patientId: patientBoundary._id });
    ...
    await recoveryService.dismissOpportunity(recInactive._id.toString(), tenantAId);

    // Run detection again — should be blocked by cooldown (just dismissed)
    const detectAfterTerminal = await recoveryService.detectRecoveryOpportunities(tenantAId);
    ...
    const recsForInactive = await Recovery.find({
      tenantId: tenantAId,
      patientId: patientInactive._id,
