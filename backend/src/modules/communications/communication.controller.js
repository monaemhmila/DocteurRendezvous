...
const getConversations = async (req, res) => {
    try {
        ...
        const conversations = await communication_model_1.Conversation.find({ tenantId })
...
const getConversationMessages = async (req, res) => {
    try {
        ...
        const conversation = await communication_model_1.Conversation.findOne({ _id: id, tenantId });
        if (!conversation) {
            return res.status(404).json({ error: "Conversation not found" });
        }
        const messages = await communication_model_1.Message.find({ tenantId, conversationId: id })
            .sort({ createdAt: 1 })
            .lean();
...
const sendMessage = async (req, res) => {
    try {
        ...
        const newMessage = await communication_model_1.Message.create({
            tenantId,
